<?php
  include_once "./libs/utils.php";
  $user = auth();
?>

<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Cyber Security</title>

    <!-- ----------- Font Awesome ------------ -->
    <link
      rel="stylesheet"
      href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css"
    />

    <!-- GOOGLE FONTS (Titillium-Web) -->
    <link rel="preconnect" href="https://fonts.googleapis.com" />
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin />
    <link
      href="https://fonts.googleapis.com/css2?family=Titillium+Web:wght@300;400;600;700;900&display=swap"
      rel="stylesheet"
    />

    <!-- GOOGLE FONTS (Poppins) -->
    <link rel="preconnect" href="https://fonts.googleapis.com" />
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin />
    <link
      href="https://fonts.googleapis.com/css2?family=Poppins:wght@200;300;400;500;600;700;800&family=Titillium+Web:wght@300;400;600;700;900&display=swap"
      rel="stylesheet"
    />

    <link rel="stylesheet" href="./css/style.css" />
    <script src="https://code.jquery.com/jquery-3.7.1.min.js" integrity="sha256-/JqT3SQfawRcv/BIHPThkBvs0OEvtFFmqPF/lYI/Cxo=" crossorigin="anonymous"></script>
  </head>

  <body>
    <!-- =========== START OF NAVBAR ============== -->
    <div class="vector1"><img src="./images/vector1.png" alt="vector" /></div>
    <div class="ellipse1"></div>

    <nav class="active">
      <div class="container nav_container">
        <a href="index.php"
          ><h4>Cyber<span>Ez</span></h4></a
        >
        <div class="navbar">
          <ul class="nav_menu" id="nav_body">
            <li>
              <a class="navbar_link" href="index.php">Home</a>
            </li>
            <li><a class="navbar_link" href="courses.php">Courses</a></li>
            <li>
              <a class="navbar_link" href="contact-us.php">Contact Us</a>
            </li>
            <li><a class="navbar_link" href="https://playfairciphertextproject.netlify.app/">Cipher Play</a></li>
            
            <li style="display: <?php echo !isset($user["id"])?"block":'none'  ?>">
              <a class="navbar_link login" href="login.php">Login</a>
            </li>
            <div class="user_profile"  style="display: <?php echo isset($user["id"])?"block":'none'  ?>">
              <div class="user_avatar" id="profileBtn">
                <img src="./images/avatar.jpg" alt="Avatar" class="abatar" />
              </div>
              <div class="toggle_menu" id="menu" style="display:none">
                <p><?php echo  $user["username"] ?></p>
                <?php 
                  if($_SERVER["REQUEST_METHOD"]=="POST"){
                    logOut();
                  }
                ?>
                <form method="POST" >
                  <button type="submit">Logout</button>
                </form>
              </div>
            </div>
          </ul>
        </div>

        <script>
        $(document).ready(function() {
          $("#profileBtn").click(function() {
              console.log($("#menu"))
                $("#menu").slideToggle();
            });
        });
    </script>
        
        <div class="mobile_navbar_btn">
          <button class="mobile_navbar_icon" name="open">
            <i class="fa-solid fa-bars"></i>
          </button>
          <button class="mobile_navbar_icon" name="close">
            <i class="fa-solid fa-xmark"></i>
          </button>
        </div>
      </div>
    </nav>

    <!-- =========== END OF NAVBAR ============== -->

    <!-- =========== START OF HEADER ============== -->
    <header>
      <div class="container header_container">
        <div class="left_header">
          <h1>Empowering You in the Digital Age</h1>
          <p>
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vestibulum
            congue metus quis accumsan euismod. Maecenas sed est mollis,
            convallis nisi convallis, imperdiet massa. Proin ipsum nunc, lacinia
            ac faucibus quis, ullamcorper non metus.
          </p>
          <a href="" class="btn_primary">Purchase</a>
        </div>
        <div class="right_header">
          <div class="right_header-image">
            <img src="./images/hero.png" alt="Hero" />
          </div>
        </div>
      </div>
    </header>

    <!-- =========== END OF HEADER ============== -->

    <!-- =========== START OF ABOUT US ============== -->
    <section class="about-us">
      <div class="container about-us_container">
        <article class="left_about-us">
          <div class="left_about-us_image">
            <img
              src="./images/about1.jpeg"
              alt="About Us"
              class="first-image"
            />
            <img
              src="./images/about2.jpeg"
              alt="About Us"
              class="second-image"
            />
          </div>
        </article>
        <article class="right_about-us right-to-left">
          <h6>About us</h6>
          <h2>
            Discover Our Journey Protecting Your Digital World with Expertise
            and Care
          </h2>
          <p>
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vestibulum
            congue metus quis accumsan euismod. Maecenas sed est mollis,
            convallis nisi convallis, imperdiet massa. Proin ipsum nunc, lacinia
            ac faucibus quis, ullamcorper non metus. Aliquam viverra volutpat
            feugiat. Donec fringilla aliquam mollis. Sed cursus quam nibh, quis
            placerat metus dapibus sed. Nam varius nisi in nisi maximus blandit
            sit amet ac tellus.
          </p>
          <a href="" class="btn_primary">Read More</a>
        </article>
        <div class="vector-right1">
          <img src="./images/vector-right1.png" alt="vector" />
        </div>
        <div class="ellipse-right1"></div>
      </div>
    </section>

    <!-- =========== END OF ABOUT US ============== -->

    <!-- =========== START OF OUR SERVICE ============== -->
    <section class="our-service">
      <div class="container our-service_container">
        <div class="vector-right2">
          <img src="./images/vector-right2.png" alt="vector" />
        </div>
        <div class="ellipse-right2"></div>
        <article class="upper_our-service">
          <div class="content-center">
            <h6>OUR SERVICE</h6>
            <h2>
              Protecting Your Digital <br />
              Assets Expertly
            </h2>
            <p>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit.
              Vestibulum congue metus quis accumsan euismod. Maecenas sed est
              mollis, convallis nisi convallis, imperdiet massa.
            </p>
          </div>
        </article>
        <article class="below_our-service">
          <div class="service">
            <div>
              <div class="service-icon"></div>
              <h5>Cyber security Assessment</h5>
              <p>
                Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                Vestibulum congue metus quis.
              </p>
            </div>
          </div>
          <div class="service">
            <div>
              <div class="service-icon"></div>
              <h5>Intrusion Detection and Prevention</h5>
              <p>
                Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                Vestibulum congue metus quis.
              </p>
            </div>
          </div>
          <div class="service">
            <div>
              <div class="service-icon"></div>
              <h5>Incident Response and Recovery</h5>
              <p>
                Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                Vestibulum congue metus quis.
              </p>
            </div>
          </div>
        </article>
        <div class="vector-left1">
          <img src="./images/vector-left1.png" alt="vector" />
        </div>
        <div class="ellipse-left1"></div>
      </div>
    </section>

    <!-- ================ END OF OUR SERVICE ====================-->

    <!--  ================ START OF FEATURE POINT ====================  -->
    <section class="feature">
      <div class="container feature_container">
        <div class="vector-left2">
          <img src="./images/vector-left2.png" alt="vector" />
        </div>
        <div class="ellipse-left2"></div>
        <article class="left_feature">
          <div class="left_feature-image">
            <img src="./images//feature.png" alt="Feature Point" />
          </div>
        </article>
        <article class="right_feature last-to-first">
          <div class="upper_right_feature">
            <h6>Feature Point</h6>
            <h2>
              Key Service Features <br />
              Protecting You
            </h2>
            <p>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit.
              Vestibulum congue metus quis accumsan euismod. Maecenas sed est
              mollis, convallis nisi convallis, imperdiet massa.
            </p>
          </div>
          <div class="below_right_feature">
            <div class="features_body">
              <div class="feature-icon">Icon</div>
              <h5>Customized Security Solutions</h5>
              <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
            </div>
            <div class="features_body">
              <div class="feature-icon">Icon</div>
              <h5>Vulnerability Assessment</h5>
              <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
            </div>
            <div class="features_body">
              <div class="feature-icon">Icon</div>
              <h5>24/7 Incident Response</h5>
              <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
            </div>
            <div class="features_body">
              <div class="feature-icon">Icon</div>
              <h5>User Training Programs</h5>
              <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
            </div>
          </div>
        </article>
        <div class="vector-right1">
          <img src="./images/vector-right1.png" alt="vector" />
        </div>
        <div class="ellipse-right1"></div>
      </div>
    </section>

    <!--  ================ END OF FEATURE POINT ====================  -->

    <!--  ================ START OF OUR GOALS ====================  -->
    <section class="our-goals">
      <div class="our-goals_container">
        <div class="vector-right2">
          <img src="./images/vector-right2.png" alt="vector" />
        </div>
        <div class="ellipse-right2"></div>
        <article class="upper_our-goals">
          <div class="content-center">
            <h6>OUR GOALS</h6>
            <h2>
              Securing Your Digital <br />
              World Together
            </h2>
            <p>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit.
              Vestibulum congue metus quis accumsan euismod. Maecenas sed est
              mollis, convallis nisi convallis, imperdiet massa.
            </p>
          </div>
        </article>
        <article class="below_our-goals">
          <div class="slider_element">
            <div class="our-goals_body">
              <h5>Mission Statement</h5>
              <p>
                Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                Vestibulum congue metus quis accumsan euismod.
              </p>
            </div>
            <div class="our-goals_body">
              <h5>Key Objectives</h5>
              <p>
                Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                Vestibulum congue metus quis accumsan euismod.
              </p>
            </div>
            <div class="our-goals_body">
              <h5>Client-Centric Approach</h5>
              <p>
                Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                Vestibulum congue metus quis accumsan euismod.
              </p>
            </div>
            <div class="our-goals_body">
              <h5>Request Action</h5>
              <p>
                Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                Vestibulum congue metus quis accumsan euismod.
              </p>
            </div>
            <div class="our-goals_body">
              <h5>Advanced upgrade</h5>
              <p>
                Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                Vestibulum congue metus quis accumsan euismod.
              </p>
            </div>
          </div>
          <div class="slider_dots"></div>
        </article>
        <div class="vector-left1">
          <img src="./images/vector-left1.png" alt="vector" />
        </div>
        <div class="ellipse-left1"></div>
      </div>
    </section>

    <!--  ================ END OF OUR GOALS ====================  -->

    <!--  ================ START OF PRICING TABLE ====================  -->
    <section class="pricing-table">
      <div class="container pricing-table_container">
        <div class="vector-left2">
          <img src="./images/vector-left2.png" alt="vector" />
        </div>
        <div class="ellipse-left2"></div>
        <article class="upper_pricing-table">
          <div class="content-center">
            <h6>Pricing Table</h6>
            <h2>
              Start Using Cyber <br />
              Security Protect
            </h2>
            <p>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit.
              Vestibulum congue metus quis accumsan euismod. Maecenas sed est
              mollis, convallis nisi convallis, imperdiet massa.
            </p>
          </div>
        </article>
        <article class="below_pricing-table">
          <div class="pricing-table_body">
            <h6 class="upper">Basic</h6>
            <p class="upper">
              Lorem ipsum dolor sit amet, consectetur adipiscing elit.
              Vestibulum congue metus quis accumsan euismod.
            </p>
            <div class="price">
              <h4>300BDT</h4>
              <span></span>
              <h6>/per month</h6>
            </div>
            <div class="service-provide">
              <div>
                <i class="fa-solid fa-check"></i>
                <p>Real-time Threat Monitoring</p>
              </div>
              <div>
                <i class="fa-solid fa-check"></i>
                <p>User Training Programs</p>
              </div>
              <div>
                <i class="fa-solid fa-check"></i>
                <p>Email Security</p>
              </div>
            </div>
            <button class="btn">Purchase</button>
          </div>
          <div class="pricing-table_body" id="standard">
            <h6 class="upper">Standard</h6>
            <p class="upper">
              Lorem ipsum dolor sit amet, consectetur adipiscing elit.
              Vestibulum congue metus quis accumsan euismod.
            </p>
            <div class="price">
              <h4>2900BDT</h4>
              <span></span>
              <h6>/per month</h6>
            </div>
            <div class="service-provide">
              <div>
                <i class="fa-solid fa-check"></i>
                <p>Everything in Basic</p>
              </div>
              <div>
                <i class="fa-solid fa-check"></i>
                <p>Vulnerability Assessments</p>
              </div>
              <div>
                <i class="fa-solid fa-check"></i>
                <p>Vulnerability Assessments</p>
              </div>
              <div>
                <i class="fa-solid fa-check"></i>
                <p>24/7 Incident Response</p>
              </div>
            </div>
            <button class="btn">Purchase</button>
          </div>
          <div class="pricing-table_body">
            <h6 class="upper">Premium</h6>
            <p class="upper">
              Lorem ipsum dolor sit amet, consectetur adipiscing elit.
              Vestibulum congue metus quis accumsan euismod.
            </p>
            <div class="price">
              <h4>3099BDT</h4>
              <span></span>
              <h6>/per month</h6>
            </div>
            <div class="service-provide">
              <div>
                <i class="fa-solid fa-check"></i>
                <p>Everything in Standard</p>
              </div>
              <div>
                <i class="fa-solid fa-check"></i>
                <p>Compliance Audits</p>
              </div>
              <div>
                <i class="fa-solid fa-check"></i>
                <p>Advanced Threat Intelligence</p>
              </div>
            </div>
            <button class="btn">Purchase</button>
          </div>
        </article>
        <div class="vector-right1">
          <img src="./images/vector-right1.png" alt="vector" />
        </div>
        <div class="ellipse-right1"></div>
      </div>
    </section>

    <!--  ================ END OF PRICING TABLE ====================  -->

    <!--  ================ START OF TESTIMONIAL ====================  -->
    <section class="testimonial">
      <div class="container testimonial_container">
        <div class="vector-right2">
          <img src="./images/vector-right2.png" alt="vector" />
        </div>
        <div class="ellipse-right2"></div>
        <article class="upper_testimonial">
          <div class="content-center">
            <h6>Testimonial</h6>
            <h2>
              see what others people <br />
              are saying
            </h2>
            <p>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit.
              Vestibulum congue metus quis accumsan euismod. Maecenas sed est
              mollis, convallis nisi convallis nisi convallis, imperdiet massa.
            </p>
          </div>
        </article>
        <article class="below_testimonial">
          <div class="testimonial_body">
            <p>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit.
              Vestibulum congue metus quis accumsan euismod. Maecenas sed est
              mollis, convallis nisi convallis nisi convallis, imperdiet massa.
            </p>
            <div class="sub_body">
              <img src="./images/sajjad.jpg" alt="Testimonial" />
              <div>
                <h5>Md Sajjad Hossen</h5>
                <p>Investor Metrics Executive</p>
                <div class="icons">
                  <i class="fa-regular fa-star"></i>
                  <i class="fa-regular fa-star"></i>
                  <i class="fa-regular fa-star"></i>
                  <i class="fa-regular fa-star"></i>
                  <i class="fa-regular fa-star"></i>
                </div>
              </div>
            </div>
          </div>
          <div class="testimonial_body">
            <p>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit.
              Vestibulum congue metus quis accumsan euismod. Maecenas sed est
              mollis, convallis nisi convallis nisi convallis, imperdiet massa.
            </p>
            <div class="sub_body">
              <img src="./images/wares.jpg" alt="Testimonial" />
              <div>
                <h5>Mr Wares</h5>
                <p>Cyber Legend</p>
                <div class="icons">
                  <i class="fa-regular fa-star"></i>
                  <i class="fa-regular fa-star"></i>
                  <i class="fa-regular fa-star"></i>
                  <i class="fa-regular fa-star"></i>
                  <i class="fa-regular fa-star"></i>
                </div>
              </div>
            </div>
          </div>
          <div class="testimonial_body">
            <p>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit.
              Vestibulum congue metus quis accumsan euismod. Maecenas sed est
              mollis, convallis nisi convallis nisi convallis, imperdiet massa.
            </p>
            <div class="sub_body">
              <img src="./images/hasib.jpg" alt="Testimonial" />
              <div>
                <h5>Md Hasibur Rahmant</h5>
                <p>Regional Branding Consultant</p>
                <div class="icons">
                  <i class="fa-regular fa-star"></i>
                  <i class="fa-regular fa-star"></i>
                  <i class="fa-regular fa-star"></i>
                  <i class="fa-regular fa-star"></i>
                  <i class="fa-regular fa-star"></i>
                </div>
              </div>
            </div>
          </div>
          <div class="testimonial_body">
            <p>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit.
              Vestibulum congue metus quis accumsan euismod. Maecenas sed est
              mollis, convallis nisi convallis, imperdiet massa.
            </p>
            <div class="sub_body">
              <img src="./images/shoaib.jpg" alt="Testimonial" />
              <div>
                <h5>Shoaib Ahmed</h5>
                <p>Cyber Specialist</p>
                <div class="icons">
                  <i class="fa-regular fa-star"></i>
                  <i class="fa-regular fa-star"></i>
                  <i class="fa-regular fa-star"></i>
                  <i class="fa-regular fa-star"></i>
                  <i class="fa-regular fa-star"></i>
                </div>
              </div>
            </div>
          </div>
        </article>
        <div class="vector-left1">
          <img src="./images/vector-left1.png" alt="vector" />
        </div>
        <div class="ellipse-left1"></div>
      </div>
    </section>

    <!--  ================ END OF TESTIMONIAL ====================  -->

    <!--  ================ START OF FOOTER ====================  -->
    <footer>
      <section class="container footer_container">
        <div class="vector-left2">
          <img src="./images/vector-left2.png" alt="vector" />
        </div>
        <div class="ellipse-left2"></div>
        <article class="left_footer">
          <a href="index.php"
            ><h4>Cyber<span>Ez</span></h4></a
          >
          <p>
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Morbi sit
            amet neque tortor.
          </p>
          <div class="socials">
            <i class="fa-brands fa-instagram"></i>
            <i class="fa-brands fa-facebook-f"></i>
            <i class="fa-brands fa-x-twitter"></i>
            <i class="fa-brands fa-youtube"></i>
          </div>
        </article>
        <article class="footer_quick-links item-center">
          <div>
            <h6 class="footer_head">Quick Links</h6>
            <ul class="quick-links">
              <li><a href="#">Our Service</a></li>
              <li><a href="#">About Us</a></li>
              <li><a href="#">Pricing</a></li>
              <li><a href="#">Testimonial</a></li>
            </ul>
          </div>
        </article>
        <article class="footer_contact-us">
          <h6 class="footer_head">Contact Us</h6>
          <div class="contact-site">
            <div>
              <i class="fa-solid fa-envelope"></i>
              <a href="#">Admin@website.com</a>
            </div>
            <div>
              <i class="fa-solid fa-location-dot"></i>
              <a href="#">Baridhara, ENG</a>
            </div>
            <div>
              <i class="fa-solid fa-phone"></i>
              <a href="#">+880 1908064229</a>
            </div>
          </div>
        </article>
        <article class="footer_newsletter">
          <h6 class="footer_head">Newsletter</h6>
          <div>
            <input type="email" name="email" placeholder="Enter your email" />
            <button class="subscribe">Subscribe</button>
          </div>
        </article>
      </section>
    </footer>

    <!--  ================ END OF FOOTER ====================  -->

    <script type="module" src="./js/main.js"></script>
  </body>
</html>